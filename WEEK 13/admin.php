...
if (isset($_GET['delete'])) {
    ...
    // Fetch the image path before deleting the record to clean up storage
    ...
    if ($stmt->execute([$delete_id])) {
        ...
        // Clean up the file if it exists in the uploads folder
...
// ==========================================
// 3. READ OPERATION (Edit Mode State Handler)
// ==========================================
$edit_product = null;
if (isset($_GET['edit'])) {
...
// ==========================================
// 4. ADVANCED MULTI-CRITERIA SEARCH & READ OPERATION
// ==========================================
$search_query = '';
if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    $search_query = trim($_GET['search']);
    
    // Check if the search input is an ID number or descriptive text string
    if (is_numeric($search_query)) {
        // Safe query execution for checking explicit Primary Key matches
        ...
        // Bulletproof case-insensitive text search matching product name OR structural category field name
        $sql = "SELECT p.*, c.name as category_name 
                FROM products p 
                LEFT JOIN categories c ON p.category_id = c.id 
    ...
    // Default load: fetch all store data elements normally
...
// Load the dynamic category selector references
...
<body class="bg-slate-50 text-slate-800">
    ...
    <main class="max-w-6xl mx-auto px-4 pb-12 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <!-- SEARCH CONTAINER AND DATA TABLE GRID CONTROL VIEW -->
        <div class="md:col-span-2 space-y-4">
            
            <!-- ENTERPRISE ENHANCEMENT: Integrated Search Panel -->
            <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <form method="GET" action="admin.php" class="flex gap-2">
                    <input type="text" name="search" value="<?= htmlspecialchars($search_query) ?>" placeholder="Search by name, ID, or category..." class="flex-1 border rounded-md p-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <button type="submit" class="bg-slate-800 text-white px-4 py-2 rounded-md text-sm hover:bg-slate-700">Search</button>
                    <?php if (!empty($search_query)): ?>
                        <a href="admin.php" class="bg-slate-200 text-slate-700 px-4 py-2 rounded-md text-sm hover:bg-slate-300 flex items-center justify-center">Clear</a>
                    <?php endif; ?>
                </form>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-x-auto">
                <table class="w-full text-left border-collapse text-sm min-w-[500px]">
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="p-8 text-center text-slate-400">No matching records found.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
